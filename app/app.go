...
func (h *homeV2) addSessionToList(sess *types.Session) {
	// Convert session to adapter for UI compatibility
	adapter := types.NewSessionAdapter(sess)
	finalizer := h.list.AddInstance(adapter)
	finalizer()
}
...
func (h *homeV2) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case clearErrorMsg:
		h.errBox.SetError(nil)
	}

	// Components don't have Update methods in current implementation
	...
	for _, sess := range h.sessions {
		if sess.Title == selected.Title {
			go func() {
				...
				// Update status
				sess.Status = types.StatusPaused
			}()
			break
		}
	}
...
func (h *homeV2) resumeSelectedSession() {
	...
	for _, sess := range h.sessions {
		if sess.Title == selected.Title && sess.Status == types.StatusPaused {
			go func() {
				ctx := context.Background()
